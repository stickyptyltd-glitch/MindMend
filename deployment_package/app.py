...
# Deployment domain (set DOMAIN in env; falls back to localhost)
YOUR_DOMAIN = os.environ.get('DOMAIN', 'localhost:8000')

# Configure the database
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get("DATABASE_URL", "sqlite:///mind_mend.db")
...
with app.app_context():
    from models import (
        User, Session, BiometricData, VideoAnalysis, Exercise,
        Assessment, Subscription, Payment, Invoice
    )
    ...
    from payment_security import payment_security
...
def register():
    if request.method == "POST":
        try:
            ...
            flash("Registration successful! Welcome to Mind Mend.", "success")
...
def dashboard():
    ...
    recent_sessions = Session.query.filter_by(user_id=current_user.id)\
        .order_by(Session.timestamp.desc()).limit(5).all()
    ...
    return render_template("dashboard.html", user=current_user, sessions=recent_sessions, stats=stats)
...
def create_checkout_session():
    ...
    try:
        # Base URL for redirects: use HTTPS for production-like domains
        scheme = 'https' if YOUR_DOMAIN not in ('localhost', 'localhost:5000', 'localhost:8000') else 'http'
    ...
    if request.method == "POST":
        try:
            ...
            new_session = Session(
                ...
                patient_name=current_user.get_full_name() if current_user.is_authenticated else patient_name,
                session_type=session_type,
                input_text=user_input,
                ai_response=ai_response,
