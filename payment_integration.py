    ...
    def create_stripe_checkout_session(self,
        ...
        try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                line_items=[{
                    'price_data': {
                        ...
                        'product_data': {
                            ...
                            'description':
                            f'Mental health therapy - {plan_type} subscription',
                            'images':
                ...
                mode='subscription',
                success_url=
                f'https://{domain}/payments/success?session_id={{CHECKOUT_SESSION_ID}}',
                cancel_url=f'https://{domain}/payments/cancel',
                customer_email=session.get('user_email')
                if session.get('user_email') else None,
    ...
    def create_paypal_payment(self, plan_type, amount, currency='AUD'):
        ...
        try:
            payment = paypalrestsdk.Payment({
                ...
                "redirect_urls": {
                    "return_url":
                    f"https://{os.environ.get('DOMAIN', 'mindmend.com.au')}/payments/paypal/success",
                    "cancel_url":
                    f"https://{os.environ.get('DOMAIN', 'mindmend.com.au')}/payments/cancel"
                },
                "transactions": [{
                    "item_list": {
                        "items": [{
                    ...
                    "description":
                    f"Mental health therapy subscription - {plan_type}"
                }]
            })
...
def payment_plans():
    ...
    plans = {
        'basic': {
            ...
            32,
            'features': [
                '2 AI therapy sessions/month', 'Basic progress tracking',
                'Crisis detection', 'Email support'
            ]
        },
        'premium': {
            'name':
            ...
            65,
            'features': [
                '4 AI therapy sessions/month', 'Video assessment',
        ...
        'enterprise': {
            ...
            130,
            'features': [
                'Unlimited AI sessions', 'Human counselor access',
                'Family accounts', 'Custom integrations', '24/7 phone support'
            ]
        }
    }
...
def create_checkout_session():
    ...
    if payment_method == 'stripe':
        result = payment_manager.create_stripe_checkout_session(
            plan_type, amount, currency)
    elif payment_method == 'paypal':
        result = payment_manager.create_paypal_payment(plan_type, amount,
                                                       currency)
    else:
...
def stripe_webhook():
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        logging.warning(
            f"Payment failed for invoice: {event['data']['object']['id']}")
...
def mobile_apple_pay():
    ...
    try:
        payment_intent = stripe.PaymentIntent.create(amount=int(amount * 100),
                                                     currency='aud',
                                                     payment_method_data={
                                                         'type': 'card',
                                                         'card': {
