...
class CounselorManager:
    ...
    def get_counselor_earnings(self, counselor_id, period='month'):
        ...
        return {
            ...
            'platform_fee': base_earnings.get(period, 0) * 0.15,  # 15% platform fee
...
def dashboard():
    ...
    dashboard_data = {
        'counselor': counselor_manager.counselors.get(counselor_id, {}),
        'earnings': earnings,
        'upcoming_sessions': [
            {
                'id': 'sess_001',
        ...
        'recent_sessions': [
            {
                ...
                'fee': 120,
                'rating': 5,
                'notes': 'Great progress on anxiety management'
            }
        ],
        'performance_metrics': {
...
def employment_opportunities():
    ...
    return render_template('counselor/employment.html', 
                         opportunities={
                             'full_time': {
...
def apply():
    ...
    if request.method == 'POST':
        ...
        counselor_manager.employment_applications[application_id] = application_data
        
        flash('Application submitted successfully! We will review and contact you within 48 hours.', 'success')
        return redirect(url_for('counselor.application_confirmation', app_id=application_id))
...
def application_confirmation(app_id):
    ...
    return render_template('counselor/confirmation.html', application=application)
...
def earnings():
    ...
    earnings_data = {
        'current_month': counselor_manager.get_counselor_earnings(counselor_id, 'month'),
        'current_week': counselor_manager.get_counselor_earnings(counselor_id, 'week'),
        'yearly_total': counselor_manager.get_counselor_earnings(counselor_id, 'year'),
        'payout_history': [
            {
                'date': '2025-07-01',
...
def update_availability():
    ...
    if counselor_id and counselor_id in counselor_manager.counselors:
        counselor_manager.counselors[counselor_id]['availability'] = data.get('availability', {})
        counselor_manager.counselors[counselor_id]['last_updated'] = datetime.utcnow().isoformat()
